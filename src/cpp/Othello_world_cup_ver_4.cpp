...
public:
    ...
    // 候補手が着手前盤面で辺ウェッジになっているか判定します。
    bool isEdgeWedgeMove(
        const Board& board,
        Bitboard moveBit,
        char player
    ) const {
        if ((board.legalMovesBits(player) & moveBit) == 0) {
            return false;
        }
    ...
    // 指定位置が属する辺と辺内インデックスを取得します。
    bool findEdge(
        int position,
        std::array<int, 8>& edge,
        int& edgePosition
    ) const {
        for (const std::array<int, 8>& candidateEdge : EDGE_POSITIONS) {
            for (int index = 0; index < BOARD_SIZE; ++index) {
                if (candidateEdge[index] == position) {
                    edge = candidateEdge;
                    edgePosition = index;
                    return true;
                }
    ...
    // 相手が空き区間端へ合法着手して角からの連続石を伸ばせるか判定します。
    bool canExtendCornerLine(
        const Board& board,
        const std::array<int, 8>& edge,
        int targetIndex,
        bool fromStart,
        char opponent
    ) const {
        Bitboard targetBit = 1ULL << edge[targetIndex];
        if ((board.occupiedBoard() & targetBit) != 0) {
            return false;
        }
        if ((board.legalMovesBits(opponent) & targetBit) == 0) {
            return false;
        }
        ...
        if (fromStart) {
            for (int index = 0; index < BOARD_SIZE; ++index) {
                Bitboard currentBit = 1ULL << edge[index];
                if ((board.playerBoard(opponent) & currentBit) == 0) {
                    break;
                }
                ++beforeCount;
            }
        ...
        Board extendedBoard = board.copy();
        extendedBoard.applyMoveBit(targetBit, opponent);

        int afterCount = 0;
        if (fromStart) {
            for (int index = 0; index < BOARD_SIZE; ++index) {
                Bitboard currentBit = 1ULL << edge[index];
                if ((extendedBoard.playerBoard(opponent) & currentBit) == 0) {
                    break;
                }
                ++afterCount;
            }
    ...
    // 複数空きの辺で相手が角から連続石を伸ばせる候補手だけを危険と判定します。
    bool isMultiEmptyEdgeGapMove(
        ...
        std::array<int, 8> edge = {};
        int edgePosition = 0;
        if (!findEdge(position, edge, edgePosition)) {
            return false;
        }

        int gapStart = edgePosition;
        while (gapStart > 0) {
            Bitboard previousBit = 1ULL << edge[gapStart - 1];
            if ((board.occupiedBoard() & previousBit) != 0) {
                break;
            }
            --gapStart;
        }

        int gapEnd = edgePosition;
        while (gapEnd < BOARD_SIZE - 1) {
            Bitboard nextBit = 1ULL << edge[gapEnd + 1];
            if ((board.occupiedBoard() & nextBit) != 0) {
                break;
            }
            ++gapEnd;
        }
        ...
        Bitboard opponentBits = board.playerBoard(opponent);
        bool startCornerLine = gapStart > 0;
        for (int index = 0; index < gapStart && startCornerLine; ++index) {
            Bitboard currentBit = 1ULL << edge[index];
            if ((opponentBits & currentBit) == 0) {
                startCornerLine = false;
            }
        }

        bool endCornerLine = gapEnd < BOARD_SIZE - 1;
        for (
            int index = BOARD_SIZE - 1;
            index > gapEnd && endCornerLine;
            --index
        ) {
            Bitboard currentBit = 1ULL << edge[index];
            if ((opponentBits & currentBit) == 0) {
        ...
        boardAfterMove.applyMoveBit(moveBit, player);

        if (
            startCornerLine
            && edgePosition != gapStart
            && canExtendCornerLine(
                boardAfterMove,
                edge,
                gapStart,
