...
constexpr double ENDGAME_PRESEARCH_RATIO = 0.20;
constexpr int ENDGAME_PRESEARCH_MAX_DEPTH = 3;
...
constexpr int EMPTY_CORNER_C_WEIGHT = -500;
constexpr int EMPTY_CORNER_X_WEIGHT = -800;
...
public:
    ...
    Score getDynamicPositionWeight(
        ...
        for (const CornerPositionCheck& check : cornerChecks) {
            bool cornerIsOwnedByStone = (
                board.playerBoard(stoneOwner) & (1ULL << check.cornerPosition)
            ) != 0;

            for (int cPosition : check.cPositions) {
                if (position == cPosition) {
                    if (cornerIsOwnedByStone) {
    ...
    Score evaluateStability(const Board& board, char player) const {
        Bitboard playerBits = board.playerBoard(player);
        Bitboard opponentBits = board.opponentBoard(player);
        Bitboard myStableBits = 0;
        Bitboard opponentStableBits = 0;
        ...
        for (const CornerCheck& check : cornerChecks) {
            ...
            Bitboard stableBits = check.cornerBit;
            Bitboard currentBit = check.horizontalShift(check.cornerBit);
            while (currentBit != 0 && (currentBit & ownerBits) != 0) {
                stableBits |= currentBit;
                currentBit = check.horizontalShift(currentBit);
            }
            currentBit = check.verticalShift(check.cornerBit);
            while (currentBit != 0 && (currentBit & ownerBits) != 0) {
                stableBits |= currentBit;
                currentBit = check.verticalShift(currentBit);
            }
            if (isMyCorner) myStableBits |= stableBits;
            else opponentStableBits |= stableBits;
        }
        return __builtin_popcountll(myStableBits) - __builtin_popcountll(opponentStableBits);
    }
...
struct TranspositionKey {
    ...
    bool operator==(const TranspositionKey& other) const {
        return blackBoard == other.blackBoard
            ...
            && disablePerfectSwitch == other.disablePerfectSwitch;
    }
};
...
struct TranspositionKeyHash {
    ...
    std::size_t operator()(const TranspositionKey& key) const {
        ...
        hashValue ^= std::hash<bool>{}(key.disablePerfectSwitch) + 0x9e3779b9 + (hashValue << 6) + (hashValue >> 2);
        return hashValue;
    }
};
...
public:
    ...
    void sortMoves(
        ...
        for (Bitboard moveBit : moveBits) {
            scoredMoves.push_back({
                moveBit,
                evaluator.evaluateMoveScoreByBit(
                    board,
                    moveBit,
                    currentPlayer
                )
    ...
    std::string chooseMove(Board& board, const std::vector<std::string>& legalActions) {
        ...
        sortMoves(board, player, legalMoveBits);
        ...
        if (emptyCount <= PERFECT_EMPTY_LIMIT) {
            Clock::time_point fullEndTime = endTime;
            Clock::time_point presearchEndTime = Clock::now()
                + std::chrono::duration_cast<Clock::duration>(
                    std::chrono::duration<double>(timeLimitSeconds * ENDGAME_PRESEARCH_RATIO)
                );
            disablePerfectSwitch = true;
            endTime = std::min(fullEndTime, presearchEndTime);
            for (int presearchDepth = 1;
                 presearchDepth <= std::min(ENDGAME_PRESEARCH_MAX_DEPTH, emptyCount)
                     && !isTimeUp();
                 ++presearchDepth) {
                RootSearchResult presearchResult = searchRoot(
                    board, legalMoveBits, presearchDepth, -INF, INF, bestMoveBit
                );
                if (timedOut || !presearchResult.hasBestMove) break;
                bestMoveBit = presearchResult.bestMoveBit;
            }
            timedOut = false;
            disablePerfectSwitch = false;
            endTime = fullEndTime;
...
private:
    ...
    RootSearchResult searchRoot(
        ...
        sortMoves(board, player, orderedMoveBits, preferredMoveBit);
    ...
    Score minimax(
        ...
        int emptyCount = board.countEmptyCells();
        if (!disablePerfectSwitch && emptyCount <= PERFECT_EMPTY_LIMIT) {
        ...
        TranspositionKey cacheKey{
            ...
            myPassCount,
            opponentPassCount,
            disablePerfectSwitch
        };
        if (legalMoveBits.empty() && board.legalMovesBits(opponentPlayer) == 0) {
        ...
        sortMoves(board, currentPlayer, legalMoveBits, preferredMoveBit);
    ...
    Score perfectPlay(
        ...
        if (legalMoveBits.empty()) {
            ...
            if (opponentMovesBits == 0) {
                Score score = finalScore(board, rootPlayer);
                storePerfectCache(cacheKey, score, 0, -INF, INF);
                return score;
            }
