...
public:
    ...
    Score evaluateAll(
        ...
        if (hasLastMove) {
            score += evaluateMoveScoreByBit(lastMoveBit);
        }
        return score;
    }
};
...
struct TranspositionKeyHash {
    ...
    std::size_t operator()(const TranspositionKey& key) const {
        ...
        hashValue ^= std::hash<Bitboard>{}(key.lastMoveBit) + 0x9e3779b9 + (hashValue << 6) + (hashValue >> 2);
        hashValue ^= std::hash<bool>{}(key.hasLastMove) + 0x9e3779b9 + (hashValue << 6) + (hashValue >> 2);
        return hashValue;
    }
};
...
public:
    ...
    std::string chooseMove(Board& board, const std::vector<std::string>& legalActions) {
        ...
        Bitboard bestMoveBit = legalMoveBits[0];
        ...
        Bitboard lastCompletedBestMoveBit = 0;
        ...
        while (depth <= maxDepth) {
            ...
            while (!isTimeUp()) {
                ...
                if (timedOut || !result.hasBestMove) {
                    break;
                }
...
private:
    // ルート局面をPVSで探索し、指定窓における最善手と評価値を返します。
    RootSearchResult searchRoot(
        ...
        return RootSearchResult{bestScore, bestMoveBit, hasBestMove};
    }
