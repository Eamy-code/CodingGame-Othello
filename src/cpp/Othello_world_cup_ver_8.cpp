...
constexpr Score ODD_EMPTY_REGION_MOVE_ORDER_BONUS = 100000;
...
public:
    ...
    void sortMoves(
        ...
        for (Bitboard moveBit : moveBits) {
            ...
            if (useEmptyRegionParity && belongsToOddEmptyRegion(board, moveBit)) {
    ...
    // 着手位置を含む空き領域のマス数が奇数か判定します。
    bool belongsToOddEmptyRegion(const Board& board, Bitboard moveBit) const {
        Bitboard emptyBits = ~board.occupiedBoard();
        Bitboard regionBits = moveBit;
        Bitboard frontierBits = moveBit;

        while (frontierBits != 0) {
            Bitboard nextBits = neighborBits(frontierBits) & emptyBits & ~regionBits;
            regionBits |= nextBits;
            frontierBits = nextBits;
        }

        return (__builtin_popcountll(regionBits) % 2) == 1;
    }
...
private:
    ...
    Score perfectPlay(
        ...
        sortMoves(board, currentPlayer, legalMoveBits, preferredMoveBit, true);
