...
| 対象ソース | `src/cpp/Othello_world_cup_ver_5.cpp` |
| 対象言語 | C++17 |
...
| 検証状況 | ver5について、コンパイル、テスト、ローカル対戦、ベンチマークは実行していない |
...
3. 空きマスが10以下の場合、可能であれば終局まで完全探索する。
...
着手順スコアは次の合計です。各補正は盤面評価関数本体には加算しません。

```text
MOVE_WEIGHTS[position]
+ 辺ウェッジ成立時のEDGE_WEDGE_MOVE_BONUS
+ 複数空き危険時のMULTI_EMPTY_EDGE_GAP_MOVE_PENALTY
```

複数空き危険判定では、次の8本の有方向辺配列を使用します。
...
各配列は`ray[0]`を判定基準の角、`ray[1]`をその角に隣接するCマス、`ray[7]`を反対側の角とします。盤面自体は回転・反転せず、この有方向配列によって8方向へ同じ判定ロジックを適用します。

各盤面位置には、対応するray番号とray内位置を`CORNER_RAY_REFERENCES`で事前対応付けします。内部マスと角は参照数0、角以外の辺マスは同じ辺を両端から見る最大2rayを保持するため、候補手ごとの実行時検索は行いません。

判定手順は次のとおりです。

1. 候補手のray内位置から角方向と反対方向へ空白を走査し、候補手を含む連続空き区間と固定長のビット集合を取得する。
2. 空き区間が2マス未満、相手が`ray[0]`の角を所有していない、または角から連続する相手石列と空き区間が直接隣接していない場合は終了する。
3. 角からの連続相手石数は`ray[0]`から最大8マスを順に調べ、空白または別プレイヤーの石で打ち切る。
4. 最大2rayへの安い静的判定を先に行い、危険候補が1件以上ある場合だけ自分の着手を1回適用する。
5. 自分着手後の相手合法手を候補手ごとに1回だけ生成し、着手前に得た空き区間内へ残る合法手だけを返信候補とする。
6. 返信用盤面を1つだけ複製し、各返信を`applyMoveBit`と`undoMove`で適用・取消する。
7. 相手返信後に基準角からの連続相手石数が実際に増えた場合だけ危険と判定する。

2方向のどちらかで危険なら`bool`値として成立させ、`MULTI_EMPTY_EDGE_GAP_MOVE_PENALTY`は候補手1件へ1回だけ適用します。空き区間が1マスの`相手・空き・相手`を扱う辺ウェッジ判定とは独立した機能です。
...
通常探索用置換表に最善手が保存されている場合は、その手を先頭へ移動します。保存済み最善手がない場合、および先頭手以外は、`MOVE_WEIGHTS`へ辺ウェッジ加点と複数空き危険減点を反映した着手順スコアの降順に並べます。
...
通常Minimax中であっても、空きマスが10以下になった時点で `perfectPlay()` へ切り替えます。
...
| `isEdgeWedgeMove` | 合法な候補手の辺ウェッジ判定 |
| `countConsecutiveStonesFromCorner` | 有方向辺配列の基準角から連続する石数を取得 |
| `collectDangerousRayCandidate` | 盤面変更なしで複数空き危険候補を収集 |
| `isMultiEmptyEdgeGapMove` | 元の空き区間内の相手返信で角列が伸びる危険手を判定 |
| `evaluatePass` | パス差評価 |
...
複数空き危険判定は、候補位置から定数時間で最大2rayだけを取得します。安い静的条件を通過した場合に限り、自分着手用の盤面コピーと着手を各1回、相手合法手生成を1回行います。相手返信は元の空き区間内に限定し、返信盤面は`applyMoveBit`と`undoMove`で再利用します。この判定専用の動的コンテナ、キャッシュ、時刻取得は使用しません。
...
本AIは、64bitビットボードによる高速な盤面操作を基盤に、序盤は定石、中盤は複数評価指標を用いた時間制限付きMinimax、終盤は空き10マス以下からの完全読みを組み合わせた構成です。通常探索にはPVS、Aspiration Window、境界種別付き置換表を使用します。
